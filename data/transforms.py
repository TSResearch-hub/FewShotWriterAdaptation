...
def get_list_augmenters(img, aug_configs, fill_value):
    ...
    for aug_config in aug_configs:
        ...
        #if np.random.rand() > aug_config["proba"]:
            continue
        if aug_config["type"] == "dpi":
            ...
            kernel_h = randint(aug_config["min_kernel"], aug_config["max_kernel"] + 1) // 2 * 2 + 1
            kernel_w = randint(aug_config["min_kernel"], aug_config["max_kernel"] + 1) // 2 * 2 + 1
            sigma = rand_uniform(aug_config["min_sigma"], aug_config["max_sigma"])
...
def apply_data_augmentation(img, da_config, fill_value=1.0):
    """
    ...
    #if da_config["proba"] != 1 and np.random.rand() > da_config["proba"]:
        return to_tensor(img) #, applied_da
